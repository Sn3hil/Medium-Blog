...
export const EditBlog = () => {
    ...
    const {thisBlog} = useContext(blogContext)
    const [title , setTitle] = useState(thisBlog?.title)
    const [content , setContent] = useState(thisBlog?.content)
    ...
    async function handleClick(){
        ...
        if(!token){
            ...
            try{
                await axios.put("https://backend.shubhamthesingh21.workers.dev/api/v1/blog",
                    {
                        id : thisBlog?.id,
                        title,
                        content
    ...
    return(
        <div className="w-screen h-screen grid grid-cols-5 overflow-y-auto">
            ...
            <div className="col-start-5 mt-20 flex justify-end pr-10">
                <CustomButton label={sentReq ? <Spinner color="fill-green-500"/> : "Save"} onclick={handleClick} className="bg-green-500 text-white rounded-2xl fixed"/>
            </div>
        </div>
    )
}